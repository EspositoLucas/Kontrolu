...
from PyQt5 import sip 
from PyQt5.QtWidgets import QWidget, QColorDialog, QDialog, QVBoxLayout, QLineEdit, QPushButton, QLabel, QMenu, QAction, QScrollArea, QTextEdit, QApplication,QComboBox,QMessageBox,QHBoxLayout,QInputDialog
from PyQt5.QtGui import QPainter, QPen, QColor, QBrush, QPixmap, QCursor,QFont,QTransform
from PyQt5.QtCore import Qt, QPointF, QRectF,QPoint,QTimer,QSize
from .micro_bloque import Microbloque
from .latex_editor import LatexEditor
from .funcion_transferencia import FuncionTransferencia
...
class DrawingArea(QWidget):
    ...
    def ajustar_tamanio_widget(self):
        ...
        scrollable_widget = self.find_scrollable_parent()
        
        if scrollable_widget:
            viewport = scrollable_widget.viewport()
            # Aumentar el factor de multiplicación para permitir más espacio
            min_width = max(content_size.width(), viewport.width() * 3)
            min_height = max(content_size.height(), viewport.height() * 3)
            
            # Añadir margen extra en la parte inferior
            extra_margin = 1000  # Ajusta este valor según sea necesario
            min_height += extra_margin
            
            self.setMinimumSize(min_width, min_height)
            scrollable_widget.adjustScrollBars()
        else:
            self.setMinimumSize(3000, 3000)  # Aumentar el tamaño predeterminado
...
#         self.update()
    ...
    def content_size_changed(self):
        new_size = self.content.size()
        self.setMinimumSize(new_size)
        self.resize(new_size)
        
        # Notificar al ScrollableWidget que debe ajustar sus barras de desplazamiento
        scrollable_widget = self.find_scrollable_parent()
        if scrollable_widget:
            scrollable_widget.adjustScrollBars()
    
    def find_scrollable_parent(self):
        parent = self.parent()
        while parent:
            if isinstance(parent, ScrollableWidget):
                return parent
...
class DrawingContent(QWidget):
    ...
    def ajustar_tamanio_widget(self):
        if self.parent():
            self.parent().ajustar_tamanio_widget()
    ...
    def paintEvent(self, event):
        ...
        # self.ensure_sufficient_size()
        
    def ensure_sufficient_size(self):
        max_x = max_y = 0
        for microbloque in self.microbloques:
            max_x = max(max_x, microbloque.pos().x() + microbloque.width())
            max_y = max(max_y, microbloque.pos().y() + microbloque.height())
        
        margin = 500
        new_width = max(self.current_size.width(), max_x + margin)
        new_height = max(self.current_size.height(), max_y + margin)
        
        if new_width > self.current_size.width() or new_height > self.current_size.height():
            self.current_size = QSize(new_width, new_height)
            self.setMinimumSize(self.current_size)
            self.resize(self.current_size)
            self.update()
            
            # Notificar al padre (DrawingArea) del cambio de tamaño
            if isinstance(self.parent(), DrawingArea):
                self.parent().content_size_changed()
