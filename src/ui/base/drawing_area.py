...
import os
from PyQt5.QtWidgets import QWidget, QColorDialog, QDialog, QVBoxLayout, QLineEdit, QPushButton, QLabel, QMenu, QAction, QScrollArea, QTextEdit, QApplication,QHBoxLayout
from PyQt5.QtGui import QPainter, QPen, QColor, QBrush, QPixmap, QCursor,QFont
from PyQt5.QtCore import Qt, QPointF, QRectF,QPoint,QTimer
from .micro_bloque import Microbloque
from .latex_editor import LatexEditor
...
class DrawingContent(QWidget):
    ...
    def create_new_microbloque(self, pos, relation=None, reference_structure=None):
        ...
        preview_button = QPushButton("Previsualización")
        preview_button.setStyleSheet("background-color: #444; color: white;")
        preview_button.clicked.connect(lambda: self.show_preview_dialog(relation, reference_structure))
    ...
    def show_preview_dialog(self, relation, reference_structure):
        preview_dialog = QDialog(self)
        preview_dialog.setWindowTitle("Previsualización")
        preview_dialog.setStyleSheet("background-color: #333; color: white;")
        layout = QVBoxLayout()

        direction = self.get_direction_from_relation(relation)
        ...
        description_label = QLabel(self.get_preview_description(direction, reference_structure))
    ...
    def get_direction_from_relation(self, relation):
    ...
    def show_add_menu(self, direction):
        ...
        for parent in [[micro_back, 0]] + parent_structures:
            ...
            action.triggered.connect(lambda checked, d=direction, s=parent[0]: 
                QTimer.singleShot(0, lambda: self.create_new_microbloque(None, d, s)))
