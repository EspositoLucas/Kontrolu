...
class Microbloque(QWidget):
    ...
    def paintEvent(self, event):
        ...
        # Dibuja el rectángulo
        if self.esta_selecionado:
            painter.setPen(QPen(Qt.red, 3 / self.escala))  # Ajustar grosor del borde
        else:
            painter.setPen(QPen(Qt.black, 2 / self.escala))
        painter.setBrush(self.color)
        painter.drawRect(QRectF(0, 0, self.ancho, self.alto))
        
        # Configura la fuente
        font = painter.font()
        font_size = 10  # Tamaño de fuente fijo
        font.setPointSizeF(font_size)
        painter.setFont(font)
        
        # Dibuja el texto
        painter.setPen(Qt.black)
        text_rect = QRectF(5 / self.escala, 5 / self.escala, self.ancho - 10 / self.escala, self.alto - 10 / self.escala)
        painter.drawText(text_rect, Qt.AlignCenter | Qt.TextWordWrap, self.nombre)
