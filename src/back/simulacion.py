...
class Simulacion:
    ...
    def preparar_sistema(self):
        """
        Prepara el sistema de control procesando cada componente por separado.
        """
        self.controlador.tf = self.procesar_topologia(self.controlador.topologia)
        self.actuador.tf = self.procesar_topologia(self.actuador.topologia)
        self.proceso.tf = self.procesar_topologia(self.proceso.topologia)
        
        
        
        print(f"TF Controlador: {self.controlador.tf}")
        print(f"TF Actuador: {self.actuador.tf}")
        print(f"TF Proceso: {self.proceso.tf}")
        print(f"Medidor: {self.medidor}")
            
    def simular_paso(self, t, dt, setpoint, y_actual):
        """
        Simula un paso del sistema de control en lazo cerrado.
        """
        # El medidor produce una señal independiente basada en la condición actual
        medicion = self.medidor.medir(y_actual)
        
        # Calcula el error usando la medición en lugar de y_actual
        error = setpoint - medicion
        
        # Aplica la función de transferencia del controlador al error
        u_controlador = self.aplicar_tf(self.controlador.tf, error)
        
        # Aplica la función de transferencia del actuador a la salida del controlador
        u_actuador = self.aplicar_tf(self.actuador.tf, u_controlador)
        
        # Aplica la función de transferencia del proceso a la salida del actuador
        y_proceso = self.aplicar_tf(self.proceso.tf, u_actuador)
        
        return y_proceso, medicion

    def aplicar_tf(self, tf, entrada):
        """
        Aplica una función de transferencia a una entrada dada.
        Ahora maneja sistemas de cualquier orden.
        """
        num, den = tf.num[0][0], tf.den[0][0]
        
        # Usa scipy.signal.lsim para simular la respuesta del sistema
        t = np.linspace(0, 0.1, 10)  # Pequeño intervalo de tiempo
        tout, yout, _ = signal.lsim((num, den), U=entrada * np.ones_like(t), T=t)
        
        return yout[-1]  # Retorna el último valor de la salida
    
    def simular_sistema_tiempo_real(self, entrada, t_total, dt, setpoint):
        """
        Simula el sistema de control en lazo cerrado en tiempo real.
        
        Parámetros:
        - entrada: Función de entrada o valor constante
        - t_total: Tiempo total de simulación
        - dt: Intervalo de tiempo entre pasos de simulación
        - setpoint: Valor de referencia
        
        Retorna:
        - t: Array de tiempos
        - y: Array de salidas del sistema
        - u: Array de entradas al sistema
        """
        if self.sistema is None:
            self.preparar_sistema()
        ...
        for i in range(1, len(t)):
            # Simula un paso del sistema y guarda los resultados
            y[i], y_medido[i] = self.simular_paso(t[i], dt, setpoint, y_medido[i-1])
